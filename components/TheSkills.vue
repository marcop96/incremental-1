...
<template>
  <div v-if="!skill.isCombat">
    ...
    <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      <ActionCard v-for="resource in filteredResources" :key="resource.id" class="cursor-pointer" :resource="resource"
        :active-resource="activeResource" @click="gatherStore.toggleGathering(resource)" />
    </div>
  </div>
</template>
